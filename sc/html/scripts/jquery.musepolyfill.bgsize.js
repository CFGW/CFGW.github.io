...
(function(a) {
  ...
  if (!Muse.Browser.Features.checkCSSFeature("background-size")) {
    var c = function(c) {
        ...
        if (c === document.body) f = a("html"), c = f.get(0), d = b(f), h.src = d, f.css("background-attachment") == "fixed" ? (j.style.position = "fixed", o = !1) : j.style.position = "absolute";
        else if (f.is("#page")) f.css("marginLeft").toLowerCase() == "auto" && (n = !0), j.style.top = f.offset().top +
          parseInt(f.css("borderTopWidth")) + "px", j.style.bottom = parseInt(f.parent().css("paddingBottom")) + parseInt(f.css("borderBottomWidth")) + "px", j.style.left = f.offset().left + parseInt(f.css("borderLeftWidth")) + "px", j.style.right = f.offset().left + parseInt(f.css("borderRightWidth")) + "px", j.style.zIndex = 0;
        else if (f.css("position") == "static") c.style.position = "relative";
        this.reloadImage = function() {
          ...
          if (f.data("hasBackgroundPositionScrollEffect") != !0)
            for (var d = 0, k = a.length; d < k; d++) switch (a[d]) {
              case "center":
              case "50%":
        ...
        this.resizeImage = function(a) {
          var b = c.getBoundingClientRect(),
            d = c.scrollWidth - (Muse.Browser.Bugs.ScrollWidthHeightIncludesBorder ? b.right - b.left - f.innerWidth() : 0),
            b = c.scrollHeight - (Muse.Browser.Bugs.ScrollWidthHeightIncludesBorder ? b.bottom - b.top - f.innerHeight() : 0),
            d = !o ? c.clientWidth : Math.max(d, c.clientWidth),
        ...
        this.update = function() {
          if (l) {
            ...
            var a = f.css("background-image").toLowerCase(),
    ...
    if (-1 != a) {
      ...
      for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
        ...
        if ("generator" == d.getAttribute("name")) {
          "2015.2.0.352" != d.getAttribute("content") && Muse.assets.outOfDate.push("jquery.musepolyfill.bgsize.js");
          break
        }
      }
    }
