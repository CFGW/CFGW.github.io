...
(function(a, b, c, d, g) {
  ...
  c.Plugins.Lightbox = {
    ...
    _initializeMarkup: function(b) {
      var d = b.options,
        ...
        p = k.outerWidth(),
        q = k.outerHeight(),
        o = b._findWidgetElements(d.lightboxPartsSelector);
      if (0 == i.length) {
        ...
        g && (t = a("<div/>"), d.elastic === "fullScreen" ? t.addClass("fullscreen") : d.elastic === "fullWidth" &&
          ...
          }), t.append(c.Utils.includeMEditableTags(l)), t.append(c.Utils.includeMEditableTags(o)),
          n.css({
            border: "none"
          }));
        i.removeAttr("id");
        var u = a("<div/>").addClass("overlayWedge").insertBefore(c.Utils.includeMEditableTags(k)[0]);
        ...
        g || l.append(c.Utils.includeMEditableTags(o));
        l.museOverlay({
          autoOpen: !1,
          $slides: k,
          $overlaySlice: n,
  ...
  c.Plugins.ContentSlideShow = {
    ...
    _updateClipElement: function(b) {
      ...
      if (d.elastic === "fullScreen") i.addClass("fullscreen");
        ...
        if (m === "absolute") b.position = m, b.left = g.css("left"), b.top =
          ...
          var p = c.Utils.getStyleSheetRulesById(c.Utils.getPageStyleSheets(), g.get(0).id);
          b.position = m;
          b.left = c.Utils.getRuleProperty(p, "left");
          b.top = c.Utils.getRuleProperty(p, "top");
          b.bottom = c.Utils.getRuleProperty(p, "bottom");
          b.right = c.Utils.getRuleProperty(p, "right")
        }
        i.css(b);
        d.transitionStyle === "fading" && m !== "fixed" && (k = l = 0);
        g.css({
          width: l + "px",
          height: k + "px"
        })
    ...
    _transformMarkup: function(a) {
      ...
      b.transitionStyle !==
        "fading" ? (b = this._updateClipElement(a), c.css({
          position: "relative",
          top: "0",
          left: "0",
          margin: "0",
    ...
    _attachBehavior: function(b) {
      ...
      if (i) {
        var p = i.$element;
        h.event === "mouseover" && p.bind("mouseenter", function() {
          var b = a(this);
          b.data("enter", !0);
          i.selectTab(p.index(b))
        });
        h.deactivationEvent === "mouseout_trigger" ? p.bind("mouseleave", function() {
          var c = a(this);
          c.data("enter", !1);
          b.slides.hidePanel(p.index(c))
        }) : h.deactivationEvent === "mouseout_both" && (p.bind("mouseleave", function() {
          var c = a(this),
            d = p.index(c),
            g = k.eq(d);
          c.data("enter", !1);
      ...
      b._csspIsImageSlideShow || (b.slides.bind("wp-panel-hide", function(b, f) {
        c.Utils.detachIframesAndObjectsToPauseMedia(a(f.panel))
    ...
    _positionImage: function(b, j, h, i, k) {
      var l = a(d),
        ...
        p = h === "fullWidth" || h === "fullScreen",
        q = h === "fullHeight" || h === "fullScreen",
        o = j == "fitContentProportionally";
      $img = a(b);
      p = p ? d.innerWidth ? d.innerWidth : l.width() : o ? $img.data("width") :
        $img.parent().closest(":not(.bc_ch_wrapper)").width();
      l = q ? d.innerHeight ? d.innerHeight : l.height() : o ? $img.data("height") : $img.parent().closest(":not(.bc_ch_wrapper)").height();
      ...
      if (p == i && l == b) m.marginTop = "0px", m.marginLeft = "0px";
      else {
        q = i;
        k = b;
        if (j == "fillFrameProportionally") {
          if (h !== "off" || i > p && b > l) j = i / p, h = b / l, j < h ? (k = b / j, q = p) : (k = l, q = i / h)
        } else if (j ==
          "fitContentProportionally" && (h !== "off" || i > p || b > l)) j = i / p, h = b / l, j > h ? (k = b / j, q = i / j) : (k = b / h, q = i / h);
        m.width = c.Utils.pixelRound(q) + "px";
        m.height = c.Utils.pixelRound(k) + "px";
        m.marginTop = c.Utils.pixelRound((l - k) / 2) + "px";
        m.marginLeft = c.Utils.pixelRound((p - q) / 2) + "px"
      }
    }
  };
    ...
    if (-1 != a) {
      ...
      for (var a = document.getElementsByTagName("meta"), b = 0, c = a.length; b < c; b++) {
        ...
        if ("generator" == d.getAttribute("name")) {
          "2015.2.0.352" != d.getAttribute("content") && Muse.assets.outOfDate.push("musewpslideshow.js");
          break
        }
      }
    }
